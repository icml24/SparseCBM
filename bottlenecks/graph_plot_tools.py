...
def make_loss_plot(ax, hist, loss_name, eps=0.01, alpha=0.5, make_train=True, make_val=True, starting_epoch=0, group_unique=False): #loss_name="logloss"
    if len(hist) < 7:
        colors = plt.rcParams['axes.prop_cycle'].by_key()['color']
    else:
...
def make_plot(
    ...
    title="Loss with different optimizers", y_label="Loss", x_label="Iterations", 
